...
import com.thrifthunter.databinding.ActivityLongSleeveCategoryBinding
...
class LongSleeveCategoryActivity : AppCompatActivity() {
    private lateinit var binding: ActivityLongSleeveCategoryBinding
    private lateinit var longSleeveCategoryViewModel: LongSleeveCategoryViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityLongSleeveCategoryBinding.inflate(layoutInflater)
    ...
    private fun getAllData() {
        ...
        val service = ApiConfig().getApiService().getProductItem("",0, 0, "longsleeve")
    ...
    private fun getIndexBarang(indeksPage:Int, name : String) {
        binding.progressBar.setVisibility(View.VISIBLE)
        val service = ApiConfig().getApiService().getProductItem(name,indeksPage, 5, "longsleeve")
        service.enqueue(object : Callback<ListItemBarang> {
            override fun onResponse(
                call: Call<ListItemBarang>,
                response: Response<ListItemBarang>
    ...
    private fun setView() {
        ...
        supportActionBar?.title = "LongSleeve"
    }
    ...
    private fun setViewModel() {
        ...
        UserPreference.getInstance(dataStore).getItems().asLiveData().observe(this) { userData ->
            ...
            longSleeveCategoryViewModel = ViewModelProvider(
                this,
                ViewModelFactory(UserPreference.getInstance(dataStore), getToken)
            )[LongSleeveCategoryViewModel::class.java]

            longSleeveCategoryViewModel.getStories().observe(this) { user ->
                if (!user.status) {
            ...
            longSleeveCategoryViewModel.item.observe(this) {
                if (it!=null) {
